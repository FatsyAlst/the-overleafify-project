...
\label{cap:8_graf_diagra}

\chapterquote{''O maior valor de uma imagem é quando ela nos obriga a notar aquilo que jamais esperávamos ver.''}{John Tukey}

A visualização de dados é essencial para a comunicação científica e técnica. O \LaTeX{} fornece poderosos pacotes para a criação de gráficos e diagramas vetoriais de alta qualidade. Este capítulo apresenta os pacotes \texttt{TikZ} e \texttt{pgfplots} \cite{mittelbach2004latex}, dando exemplo de como utilizá-los para produzir representações gráficas eficazes.

% --------

\section{Introdução ao \texttt{TikZ} e \texttt{pgfplots}}
\label{sec:cap8_introdução_ao_tikz_e_pgfplots} % Referência cruzada para esta seção
...
Apresentaremos uma série de exemplos de diagramas, imagens e gráficos que podem ser criados no \LaTeX{} utilizando o pacote TikZ. Embora os exemplos sirvam como ponto de partida, não se espera que o leitor compreenda ou consiga reproduzi-los perfeitamente já na primeira tentativa. A proposta é que o leitor escolha uma figura de interesse, tente replicá-la e modifique seus parâmetros para entender melhor o funcionamento de cada componente.

Para mais exemplos e informações detalhadas, consulte o \href{https://tikz.dev/}{Manual do \texttt{TikZ}}. 
...
\begin{lstlisting}[language=tex, caption=Diagramas de fluxo]
    \begin{center}
        \begin{tikzpicture}
            ...
            % Nós/Figuras do diagrama
            \node (start) at (-2,0) [draw, rectangle] {Início};
            % O primeiro parametro é o label, nesse exemplo seria o Start.
            \node (step1) at (0,0) [draw, circle] {Passo 1};
            % O Segundo parametro seria a localização do nó, nesse nó seria na origem.
            \node (end) at (2,0) [draw, rectangle] {Fim};
            % O terceiro parametro seria o desenho com o conteudo escrito dentro dele, nesse caso seria um retangulo com a inscrição Fim dentro dele.
            

            % Conexões com setas
            % Mostra o inicio e destino da conexão
            \draw[->] (start) -- (step1);
            \draw[->] (step1) -- (end);
        \end{tikzpicture}
    \end{center}
...
\begin{lstlisting}[language=tex, caption=Superfície 3D de função matemática]
    \begin{center}
        \begin{tikzpicture}
            \begin{axis}[
                view={60}{30}, % Angulo de visualizacao (azimute, elevacao)
                xlabel = \(x\),
                ylabel = \(y\),
                zlabel = \(z\),
...
\begin{lstlisting}[language=tex, caption=Gráfico de contorno 3D]
    \begin{center}
        \begin{tikzpicture}
            \begin{axis}[
                view={45}{30}, % Angulo ajustado para melhor visualizacao
                ...
                colorbar style={
                    ylabel={Temperatura (graus C)}, % Rotulo da escala
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            colorbar, % Adiciona barra lateral de cores
            colorbar style={
                ylabel={Temperatura}, % Rótulo da escala
                ytick={-1,0,1,2,3,4}, % Marcas da barra de cores
            }
        ]
            % Superfície 3D ondulada com interpolação de cores
...
Depois de dominar a criação e personalização de gráficos e diagramas, no Capítulo~\ref{cap:9_beamer(PPX_latec)} você aprenderá como construir apresentações dinâmicas e profissionais com o Beamer, transformando suas ideias em slides impactantes.